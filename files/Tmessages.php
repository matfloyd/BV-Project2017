...
if (isset($_POST['submit'])) {
	...
	if ($trimmed['PID']) {
		...
		$fname =  mysqli_real_escape_string($dbConnection, explode(" ", $trimmed['PID'])[0]);

		$lname = mysqli_real_escape_string($dbConnection, explode(" ", $trimmed['PID'])[1]);
	...
	if($SID){
		$PIDquery = $dbConnection->query("Select ParID from Student where SID = '$SID';");

		$queryArray2 = mysqli_fetch_array($PIDquery);

		$PID = $queryArray2['ParID'];
